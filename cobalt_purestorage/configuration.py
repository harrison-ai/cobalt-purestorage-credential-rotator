...
class Settings(BaseSettings):
    ...
    excluded_user_names: set[str] = Field(
        set(),
        env="EXCLUDED_USER_NAMES",
        description="Object Store user names within accounts of interest to exclude from rotation",
    )
    interesting_object_accounts: set[str] = Field(
        set(),
        env="INTERESTING_OBJECT_ACCOUNTS",
        description="List of Object Store accounts to rotate credentials for",
    )
    k8s_mode: bool = Field(
